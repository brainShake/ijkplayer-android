 ...
 * <p>
 * JNI 通过 {@code RegisterNatives} 绑定（jni4android），不是标准 {@code Java_*} 符号，
 * IDE 里从 {@code native} 方法无法直接 Ctrl+点击到 C。
 * 请查 {@code android/ijkplayer/docs/JNI_INDEX.md}，或在
 * {@code ijkmedia/ijkplayer/android/ijkplayer_jni.c} 的 {@code g_methods[]} 中
 * 按 Java 方法名 / {@code IjkMediaPlayer_*} 函数名搜索。
 *
 * @author bbcallen
 */
public final class IjkMediaPlayer extends AbstractMediaPlayer {
     ...
     */
    private static void loadLibraryDirect(String libName) throws UnsatisfiedLinkError, SecurityException {
        try {
            ...
            try {
                ...
                app = (Context) at.getMethod("currentApplication").invoke(null);
            } catch (Exception ignored) {
            }
            if (app != null) {
                try {
                    IjkNativeLoader.load(app, libName);
                    return;
    ...
    /**
     * 进程内只加载一次 native 库。顺序不可颠倒：ffmpeg 被 sdl/player 依赖。
     * 加载 ijkplayer 时会触发 JNI_OnLoad，完成 native 方法与 Java 的绑定。
     */
