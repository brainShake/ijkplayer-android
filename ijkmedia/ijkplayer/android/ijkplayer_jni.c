/*
 * ijkplayer_jni.c — Android JNI 主入口（与 Java IjkMediaPlayer 一一对应）
 *
 * 【框架角色】
 *   - so 加载时 JNI_OnLoad：RegisterNatives 注册 g_methods[]，初始化 FFmpeg/全局回调
 *   - 每个 Java native 方法在本文件有静态实现（如 IjkMediaPlayer_prepareAsync）
 *   - Java 对象通过字段 mNativeMediaPlayer 持有 native IjkMediaPlayer* 指针
 *
 * 【与其它模块关系】
 *   - ijkplayer_android.c：创建带 Android Surface/MediaCodec 管线的 IjkMediaPlayer
 *   - ijkplayer.c / ff_ffplay.c：平台无关播放逻辑
 *   - ijkj4a (J4AC_*)：jni4android 生成的 Java 字段/方法调用封装
 *   - message_loop 线程：把 FFplay 的 FFP_MSG_* 转成 postEventFromNative 到 Java
 *
 * 【阅读提示】
 *   - Java↔C 对照：android/ijkplayer/docs/JNI_INDEX.md
 *   - JNI 与 Java 协作：android/ijkplayer/docs/JNI_ARCHITECTURE.md
 *   - 本模块目录与线程：ijkmedia/ARCHITECTURE.md
 *
...
/* Java 类全限定名，用于 FindClass / RegisterNatives */
...
/* =============================================================================
 * 一、JNI 辅助：mNativeMediaPlayer / DataSource / AndroidIO 与 native 指针同步
 * ============================================================================= */

static JavaVM* g_jvm;  /* 供非 Java 线程通过 SDL_JNI_SetupThreadEnv 附加到 JVM */

/* 缓存的 IjkMediaPlayer jclass，RegisterNatives 时使用 */
...
/* FFplay/IO 层注入 Java 的回调（如 WillHttpOpen）；硬解选择回调 */
...
/* 从 Java 对象读取 mNativeMediaPlayer，并 ijkmp_inc_ref 防止使用中被释放 */
...
/* 写入 Java mNativeMediaPlayer；若已有旧指针则 dec_ref */
...
/* =============================================================================
 * 二、数据源：URL+Header / fd / IMediaDataSource / IAndroidIO
 * ============================================================================= */
...
/* =============================================================================
 * 三、Surface 与播放控制：prepare / start / pause / seek / stop / 查询进度
 * ============================================================================= */
...
/* =============================================================================
 * 五、选项与属性：FFmpeg option、codec 信息、MediaMeta、Property、流选择
 * ============================================================================= */
...
/* =============================================================================
 * 六、实例创建：native_init / native_setup / native_finalize
 * ============================================================================= */
...
/*
 * Java 构造时调用：创建 native 播放器并绑定 weak_this，供后续 postEventFromNative 使用。
 * message_loop 作为 msg_loop 回调，在独立线程中把 FFP_MSG 转成 Java 事件。
 */
static void
...
/* =============================================================================
 * 七、跨线程回调：IO 注入 inject_callback、硬解选择 mediacodec_select_callback
 *    （可在 read_thread 等 native 线程触发，内部再 Attach JVM）
 * ============================================================================= */
...
/* =============================================================================
 * 八、消息泵：message_loop 线程，FFP_MSG_* → postEventFromNative → Java Handler
 * ============================================================================= */
...
/*
 * 消息泵核心：阻塞读取 ijkmp 消息队列，将 FFP_MSG_* 映射为 MEDIA_* 事件到 Java。
 * 运行在 message_loop 线程，与 FFplay 解码线程分离，避免在解码线程直接调 JNI。
 */
...
/* =============================================================================
 * 九、调试与工具：systrace profile、日志级别、setFrameAtTime 抽帧
 * ============================================================================= */
...
/* =============================================================================
 * 十、JNI 注册：g_methods[] 与 JNI_OnLoad / JNI_OnUnload
 * ============================================================================= */
/*
 * Java native 方法注册表（jni4android / RegisterNatives，非 Java_* 符号）。
 * 格式：{ "Java方法名", "JNI签名", C函数指针 }
 * 完整列表见 android/ijkplayer/docs/JNI_INDEX.md
 */
...
/*
 * libijkplayer.so 加载时由 VM 调用：注册 JNI、初始化全局播放器与 FFmpeg JNI 桥。
 */
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    /* FindClass 返回局部引用；RegisterNatives 绑定 g_methods */
