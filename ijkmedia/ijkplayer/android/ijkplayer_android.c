/*
 * ijkplayer_android.c — Android 平台播放器创建与 Surface/音量绑定
 *
 * 【职责】
 *   - ijkmp_android_create：在通用 ijkmp_create 之上挂载 Android 专用组件
 *   - SDL_VoutAndroid：视频渲染到 Surface / Texture
 *   - ffpipeline_create_from_android：解码管线（含 MediaCodec 硬解节点）
 *   - ijkmp_android_set_surface / set_volume：供 ijkplayer_jni.c 中的 JNI 方法调用
 *
 * 【调用链】典型入口为 ijkplayer_jni.c 的 IjkMediaPlayer_native_setup()
 *
...
/* 创建 Android 播放器：vout + pipeline，msg_loop 一般为 ijkplayer_jni.c 的 message_loop */
