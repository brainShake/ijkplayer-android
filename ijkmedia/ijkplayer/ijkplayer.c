/*
 * ijkplayer.c — IjkMediaPlayer 状态机与 ijkmp_* API 实现
 *
 * 【职责】
 *   - 维护 mp_state（IDLE→INITIALIZED→PREPARING→PREPARED→STARTED…）
 *   - 将 Java/JNI 的 prepare/start/seek 等转为对 FFPlayer 的 ffp_* 调用
 *   - 引用计数 ijkmp_inc_ref / ijkmp_dec_ref，与 jni 层 mNativeMediaPlayer 生命周期配合
 *
 * 【注意】非法状态调用返回 EIJK_INVALID_STATE；播放细节在 ff_ffplay.c。
 *
...
/* =============================================================================
 * 一、内部销毁与全局初始化（对应 JNI_OnLoad / 进程退出）
 * ============================================================================= */
...
/* =============================================================================
 * 三、播放 API：setDataSource → prepareAsync → start/pause/stop/seek/查询
 * ============================================================================= */
...
/* =============================================================================
 * 四、循环播放与消息队列（供 JNI message_loop 消费 FFP_MSG）
 * ============================================================================= */
