/*
 * ff_ffplay.h — FFplay 播放引擎对外 API（ffp_*）
 *
 * 【职责】
 *   - 解复用、音视频解码调度、packet/frame 队列、主从时钟同步
 *   - 通过 pipeline 创建解码节点（软解 FFmpeg / Android MediaCodec）
 *   - 通过 SDL_Vout / Aout 输出；不直接调用 JNI
 *
 * 【关系】IjkMediaPlayer.ffplayer 指向 FFPlayer；ijkmp_* 在 ijkplayer.c 中封装本文件接口。
 * 架构见 ijkmedia/ARCHITECTURE.md
 *
